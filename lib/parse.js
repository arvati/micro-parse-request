...
const sign = (value, privateKey, secret) => new Promise((resolve, reject) => {
        try {
            key = new cryptoKeys('pem', privateKey);
            if (!key.isPrivate) throw new Error('Cannot sign with public key')
            if (key.isEncrypted) {
                if (!secret) throw new Error('Need Secret for encrypted private key')
                if (!key.decrypt(secret)) throw new Error('Not possible to decrypt private key')
            }
            Crypt.make({key: key.pem, is_private: true}, (err, signer) => {
                if (err) reject(err);
                signer.sign(value, (err, signed) => {
                    if (err) reject(err);
                    resolve(signed.data.replace(/\=+$/, '') + '.' + signed.signature.replace(/\=+$/, ''))
                });
            });
        } catch (e) {
            reject(e)
...
const verify = (value, publicKey) => new Promise((resolve, reject) => {
    try {
        key = new cryptoKeys('pem', publicKey);
        if (key.isPrivate) throw new Error('Cannot verify with private key')
        var data = value.slice(0, value.lastIndexOf('.'))
        var signature = value.slice(value.lastIndexOf('.')+1)
        Crypt.make({key: key.pem, is_public: true}, (err, verifier) => {
            if (err) reject(err);
            verifier.verify({data, signature}, (err, verified) => {
                if (err) reject(err);
                resolve(verified)
            });
        });
